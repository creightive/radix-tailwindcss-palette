...
export class Aliaser {
  ...
  #radixColorNames: Array<string> = Object.keys(radix).filter(color => !color.includes("Dark"));
  #rootSelector: string;
   ...
   */
  public alias(options: AliasOptions, step?: RadixStep): string | Record<string, string> {
    if (step) {
    ...
    const variableName = this.#generatePrefix("name" in options ? options.name : undefined);
    this.#aliases.set(variableName, {
      light: options.light,
      dark: options.dark,
    });

    if (!this.#isRadixColorName(options.light) || !this.#isRadixColorName(options.dark)) {
  ...
  public generateStyles({
    ...
    for (const [variableName, { light, dark }] of this.#aliases) {
      if (!this.#isRadixColorName(light)) {
        ...
        lightTable[`--${variableName}`] = themeFn(`colors.${light}`, `var(--${color}${step})`);
      } else {
        for (let i = 1; i <= steps.length; i++) {
          lightTable[`--${variableName}-${i}`] = themeFn(`colors.${light}.${i}`, `var(--${light}${i})`);
        }
      }
      ...
      if (!this.#isRadixColorName(dark)) {
        ...
        darkTable[`--${variableName}`] = themeFn(`colors.${dark}`, `var(--${color}${step})`);
      } else {
        for (let i = 1; i <= steps.length; i++) {
          darkTable[`--${variableName}-${i}`] = themeFn(`colors.${dark}.${i}`, `var(--${dark}${i})`);
        }
      }
    }
