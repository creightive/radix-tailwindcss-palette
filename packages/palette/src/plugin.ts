...
export function createPlugin({ colors = radix, opacitySupport = false, rootSelector = ":root" }: PluginOptions = {}) {
  ...
  const wrpPlugin = plugin(({ addBase, addVariant, config, theme }) => {
    ...
    const [darkMode, className = ".dark"] = ([] as Array<string>).concat(darkModeConfig);

    addVariant("p3", "@supports (color: color(display-p3 1 1 1)) { @media (color-gamut: p3) }");
...
export function generateBaseStyles(options: { colors: Partial<typeof radix>; opacitySupport: boolean }): {
  light: Record<string, string>;
  dark: Record<string, string>;
  lightP3: Record<string, string>;
  darkP3: Record<string, string>;
} {
  const mappings: Record<"light" | "dark" | "lightP3" | "darkP3", Record<string, string>> = {
    light: {},
    dark: {},
    lightP3: {},
    darkP3: {},
  ...
  for (const [colorName, steps] of Object.entries(options.colors)) {
    ...
    for (const [key, value] of Object.entries(steps)) {
      ...
      if (options.opacitySupport && colorName.includes("P3")) {
        ...
        varName = `${normalizedColorName}${key.slice(normalizedColorName.length - "P3".length)}`;
      }
...
export function generateTailwindConfig({
  ...
  for (const [colorName, steps] of Object.entries(colors)) {
    ...
    for (const key of Object.keys(steps)) {
      const stepKey = key.replace(colorName.replace("Dark", "").replace("P3", ""), "");
      let value = `var(--${key})`;
      if (opacitySupport) {
        if (colorName.includes("P3")) {
